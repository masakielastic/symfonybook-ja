...
![アクションをキャッシュする](/images/book/F1201.png "アクションをキャッシュする")
...
この設定は、キャッシュが`list`アクションのためにオンになっていること、およびレイアウトがアクション(デフォルトのふるまい)によってキャッシュされないことを保証します。このことは、キャッシュ済みのアクションが存在する場合でも、(部分テンプレートとコンポーネントと一緒に)レイアウトは再度処理されることを意味します。`with_layout`設定が`trueに設定された場合は、レイアウトはアクションによってキャッシュされ、再度処理されることはありません。
...
![部分テンプレート、コンポーネント、コンポーネントスロットをキャッシュする](/images/book/F1202.png "部分テンプレート、コンポーネント、コンポーネントスロットをキャッシュする")
...
![テンプレートのフラグメントをキャッシュする](/images/book/F1203.png "テンプレートのフラグメントをキャッシュする")
...
![キャッシュを使うページのためのWebデバッグツールバー](/images/book/F1204.png "キャッシュを使うページのためのWebデバッグツールバー")
...
![ページのキャッシュされた要素を識別する](/images/book/F1205.png "ページのキャッシュされた要素を識別する")
